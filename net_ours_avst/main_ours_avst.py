...
from net_ours_avst.dataloader_ours_avst import *
from net_ours_avst.net import AVQA_Fusion_Net
import ast
import json
import numpy as np
import pdb
...
writer = SummaryWriter('runs/net_ours_avst/'+TIMESTAMP)
...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        # print("correct qa: ", 100 * correct_qa / total_qa)
        loss = loss_qa + 0.5*loss_match

        writer.add_scalar('data/match',loss_match.item(), epoch * len(train_loader) + batch_idx)
        writer.add_scalar('data/qa_test',loss_qa.item(), epoch * len(train_loader) + batch_idx)
...
def eval(model, val_loader,epoch):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            # _, predicted = torch.max(preds_match.data, 1)
            # total_match += preds_match.size(0)
            # correct_match += (predicted == match_label).sum().item()

    print('Accuracy qa: %.2f %%' % (100 * correct_qa / total_qa))
    writer.add_scalar('metric/acc_qa',100 * correct_qa / total_qa, epoch)
    # print('Accuracy match: %.2f %%' % (100 * correct_match / total_match))
    # writer.add_scalar('metric/acc_match',100 * correct_match / total_match, epoch)
...
def main():
    ...
    parser.add_argument(
        "--model_save_dir", type=str, default='./net_ours-avst/models_ours_avst/', help="model save dir")
    parser.add_argument(
        "--checkpoint", type=str, default='models_ours_avst', help="save model name")
    parser.add_argument(
    ...
    if args.mode == 'train':
        ...
        print("\n-------------- load pretrained models finished --------------")
