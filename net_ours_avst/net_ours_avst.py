...
class AVQA_Fusion_Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.relu3 = nn.ReLU()
        self.fusion_fc4 = nn.Linear(128, 2)
        self.relu4 = nn.ReLU()
    ...
    def forward(self, audio, visual_posi, visual_nega, question):  
        ...
        # ============================== vsiaul posi feat start ==============================
        # visual posi shape: [2*B*T, 512, 14, 14]
        ...
        visual_feat_posi = nn.functional.normalize(v_feat, dim=2)

        ## audio-visual grounding positive
        audio_feat_aa = audio_feat.unsqueeze(-1)                        # [320, 512, 1]
        audio_feat_aa = nn.functional.normalize(audio_feat_aa, dim=1)
        x2_va = torch.matmul(visual_feat_posi, audio_feat_aa).squeeze()

        x2_p = F.softmax(x2_va, dim=-1).unsqueeze(-2)                       # [320, 1, 196]
        visual_feat_grd = torch.matmul(x2_p, visual_feat_posi)
        visual_feat_grd_after_grounding_posi = visual_feat_grd.squeeze()    # [320, 512]   
        ...
        visual_feat_grd_posi = self.fc_gl(visual_feat_grd)   # [B*T, 512]

        # combine audio and visual
        feat = torch.cat((audio_feat, visual_feat_grd_posi), dim=-1)     # [320, 1024]
        feat = F.relu(self.fusion_fc1(feat))       # (1024, 512)
        feat = F.relu(self.fusion_fc2(feat))       # (512, 256)
        feat = F.relu(self.fusion_fc3(feat))       # (256, 128)
        out_match_posi = self.fusion_fc4(feat)     # (128, 2)

        # ============================== vsiaul positive feat end ==============================


        # ============================== vsiaul negative feat start ==============================
        # visual nega shape:
        B, T, C, H, W = visual_nega.size()
        temp_visual = visual_nega.view(B*T,C,H,W)
        v_feat = self.avgpool(temp_visual)
        visual_feat_before_grounding_nega = v_feat.squeeze() # [B*T, 512]
        ...
        # audio-visual grounding negative
        x2_va = torch.matmul(visual_feat_nega, audio_feat_aa).squeeze()

        x2_p = F.softmax(x2_va, dim=-1).unsqueeze(-2)                       # [320, 1, 196]
        visual_feat_grd = torch.matmul(x2_p, visual_feat_nega)
        visual_feat_grd_after_grounding_nega = visual_feat_grd.squeeze()    # [320, 512]   

        visual_gl = torch.cat((visual_feat_before_grounding_nega,visual_feat_grd_after_grounding_nega),dim=-1)
        visual_feat_grd = self.tanh(visual_gl)
        visual_feat_grd_nega = self.fc_gl(visual_feat_grd)      # [B*T 512]
        
        #audio_all,visual_all,match_label=batch_organize(audio_feat,visual_feat_grd_posi,visual_feat_grd_nega)

        # combine audio and visual
        feat = torch.cat((audio_feat, visual_feat_grd_nega), dim=-1)     # [320, 1024]
        feat = F.relu(self.fusion_fc1(feat))       # (1024, 512)
        feat = F.relu(self.fusion_fc2(feat))       # (512, 256)
        feat = F.relu(self.fusion_fc3(feat))       # (256, 128)
        out_match_nega = self.fusion_fc4(feat)     # (128, 2)
